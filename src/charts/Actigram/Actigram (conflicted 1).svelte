<script>
  ...
  $: {
    days =
      1 +
      Math.floor(Math.max(...xValsToPlot.flat()) / (periodHrs * doublePlot));
    console.log(
      "DAYS = " +
        1 +
        Math.floor(Math.max(...xValsToPlot.flat()) / (periodHrs * doublePlot))
  ...
  function createActiPath(ys, sourceI, d) {
    ...
    const pathData = ys
      .filter(
        (value) =>
          value >= d * periodHrs && value <= (d + 1) * (periodHrs * doublePlot)
      ...
      .map((y, yi) => {
        ...
        return `${x - halfbarwidth},${ydayoffset + dayHeight}  
        ${x - halfbarwidth},${yout} 
        ${x + halfbarwidth},${yout} 
        ${x + halfbarwidth},${ydayoffset + dayHeight}`;
      })
      .join(" ");

    // Ensure the path starts at [0, 0]
    const moveToStart = `0,${d * (dayHeight + betweenHeight) + dayHeight}`;

    // Ensure the path returns to [0, 0] from [periodHrs, 0]
    const moveToReturn = `
    ${xScale(periodHrs * doublePlot)},${
